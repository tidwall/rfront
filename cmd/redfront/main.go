...
func main() {
	...
	config.tokens = make(map[string]aclToken)
	for i, acl := range config.ACL {
		var allow bool
		switch acl.Access {
		case "allow":
			allow = true
	...
	if len(config.Hosts) > 0 {
		...
		log.Printf("Listening at http://0.0.0.0:%d", config.Port)
		s := makeServer(fmt.Sprintf(":%d", config.Port))
...
func (s *hserver) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	pc, err := NewProxyClient(w, r, s.config)
	if err != nil {
...
func NewProxyClient(
	w http.ResponseWriter, r *http.Request, config *Config,
) (*ProxyClient, error) {
	q := r.URL.Query()
	...
	pc := &ProxyClient{
		...
		config:  config,
		cluster: append([]string{}, config.Cluster.Addrs...),
	}
	return pc, nil
}
...
func (pc *ProxyClient) allow(commandName string) bool {
	acltok, ok := pc.config.tokens[pc.auth]
	if !ok {
		return false
	}
	if acltok.allow {
		if acltok.except[strings.ToLower(commandName)] {
			return false
		}
	} else {
		if !acltok.except[strings.ToLower(commandName)] {
...
func (pc *ProxyClient) execCommand(args []string) error {
	...
	for time.Since(start) < writeTimeout {
		if pc.rc == nil {
			...
			if pc.config.Cluster.Auth != "" {
				auth := redcon.AppendArray(nil, 2)
				auth = redcon.AppendBulkString(auth, "AUTH")
				auth = redcon.AppendBulkString(auth, pc.config.Cluster.Auth)
